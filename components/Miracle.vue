  ...
  >
    ...
    >
      ...
      <div class="flex flex-1 flex-col text-[.7rem] font-bold text-black">
        <a class="line-clamp-2" :href="state.url" target="_blank">{{
          state.title
        }}</a>
      </div>
      <audio
        ...
        controls
        class="hidden"
        src="/Sea of Tranquility_Audio_128k.m4a"
        :autoplay="false"
      />
...
watch(space, (v) => v && !isInFunds() && togglePlay());

function isInFunds() {
  return location.pathname.includes('/funds');
}
